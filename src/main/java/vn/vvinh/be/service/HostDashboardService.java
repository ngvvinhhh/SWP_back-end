...
public class HostDashboardService {
    ...
    public HostDashboardDTO getDetail() {
        ...
        for (int i = 0; i <= 11; i++) {
            List<Order> orders = orderRepository.findOrderByHostAndDateBetweenAndStatus(currentAccount.getCurrentAccount(),startDate.get(i), endDate.get(i), OrderStatus.ACCEPT);
        ...
        return new HostDashboardDTO(totalRevenue, monthlyOrder, monthlyRevenue, totalOrders, totalGuest);
    }
}
